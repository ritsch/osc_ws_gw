...
parser.add_argument(
    "--oscport", action="store", dest="oscport",
    default=3333, type=int, help="TCP/IP port, default: 3333")

parser.add_argument("-t", "--tcp_ip", action="store_true", dest="tcp_ip",
    help="use TCP/IP instead of UDP")
...
if(args.tcp_ip):
    print(f"recv <- udp {args.oscaddr}:{args.oscport} <- ws://{args.wshost}:{args.wsport}")
else:   
...
if not args.tcp_ip:
    ...
    oscstream = OSCstream(addr=args.oscaddr, port=args.oscport, verbose=args.verbose)

    # websocket server
    relayserver = WebSocketRelayServer.create(loop, addr=(args.wshost, args.wsport), sender=oscstream, verbose=args.verbose)    
